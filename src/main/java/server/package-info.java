 ...
 * <ul>
 *   <li>{@link server.Server} - The main class that starts the HTTP server
 *   and sets up request handlers.</li>
 *   <li>{@link server.Server.RootHandler} - Handles HTTP GET requests to serve
 *   static files from the specified web root directory.</li>
 *   <li>{@link server.Server.UploadHandler} - Handles HTTP POST requests
